...
namespace Foodbook.Business.Services
{
    public class ShoppingListService : IShoppingListService
    {
        ...
        public async Task<ShoppingListResult> GenerateSmartShoppingListAsync(IEnumerable<Recipe> selectedRecipes, int userId)
        {
            ...
            var allIngredients = new Dictionary<string, decimal>();
            var ingredientUnits = new Dictionary<string, string>();
            var ingredientCategories = new Dictionary<string, string>();

            // Analyze all recipes and aggregate ingredients
            foreach (var recipe in recipes)
            {
                ...
                {
                    var key = ri.Ingredient.Name.ToLower();
                    if (allIngredients.ContainsKey(key))
                    {
                        allIngredients[key] += ri.Quantity;
                    }
                    else
                    {
                        allIngredients[key] = ri.Quantity;
                        ingredientUnits[key] = ri.Ingredient.Unit;
                        ingredientCategories[key] = CategorizeIngredient(ri.Ingredient.Name);
                    }
                }
            }

            // Check user's current stock
            ...
            {
                var userHasStock = userStock.Any(us => 
                    us.Name.ToLower() == ingredient.Key && us.Quantity >= ingredient.Value);

                if (!userHasStock)
                {
                    var neededQuantity = ingredient.Value;
                    var userStockItem = userStock.FirstOrDefault(us => 
                        us.Name.ToLower() == ingredient.Key);

                    if (userStockItem != null)
                    {
                        neededQuantity = Math.Max(0, ingredient.Value - userStockItem.Quantity);
                    }
                    ...
                    if (neededQuantity > 0)
                    {
                        ...
                        {
                            Name = CapitalizeFirst(ingredient.Key),
                            Quantity = neededQuantity,
                            Unit = ingredientUnits[ingredient.Key],
                            Category = ingredientCategories[ingredient.Key],
                            IsEssential = IsEssentialIngredient(ingredient.Key),
                            IsOptional = IsOptionalIngredient(ingredient.Key),
                            EstimatedPrice = CalculateEstimatedPrice(ingredient.Key, neededQuantity),
                            Notes = GenerateShoppingNotes(ingredient.Key),
                            Substitutions = GetSubstitutions(ingredient.Key)
                        };
